...
experiment_colors = {'black', 'red', 'green', 'blue'};
...
for e = 1:length(experiments)
    for block = 1:3
        ...
        block_1_u = data.(experiments{e})(block).u;
        block_1_x = data.(experiments{e})(block).X;
        block_1_y = data.(experiments{e})(block).Y;
        
        [u_dx, u_dy] = gradient(block_1_u, mean(diff(block_1_x(1,:))) * 80);
        u_dx(abs(u_dx) > 0.03) = nan;
    
    
        
        plot(block_1_x(1,:), filloutliers(u_dx(278, :), 'spline'), 'color', experiment_colors{e}, 'displayname', experiments{e})
    end
end
hold off
yline(0, 'linewidth', 3)
...
% Plot combiend data
for b = 1:length(blocks)
    dat = imgaussfilt(data.(experiment)(b).(component), 3);
    dat = (u_inf - dat) / u_inf;
    dat(dat < 0.01) = nan;
    % dat(dat > 0.99 * u_inf) = nan;
    contourf(data.(experiment)(b).X, data.(experiment)(b).Y, dat, 500, 'linestyle', 'none')
    clear b
end
